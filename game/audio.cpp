...
#if defined(__APPLE__) || (defined(__linux__) && !defined(ANDROID)) || defined (__WIIU__) || defined (__PSVITA__)
#include <SDL2/SDL.h>
#else
#include <SDL.h>
#endif
 ...
 */
#define SDL_AUDIO_ALLOW_CHANGES SDL_AUDIO_ALLOW_FREQUENCY_CHANGE|SDL_AUDIO_ALLOW_CHANNELS_CHANGE//SDL_AUDIO_ALLOW_ANY_CHANGE
...
void initAudio(void)
{
    ...
    if(gDevice == NULL)
    {
        printf("Fatal Error: Memory c-allocation error\n");
        return;
    }
    ...
    {
        printf("Error: SDL_INIT_AUDIO not initialized\n");
        return;
    }
    ...
    {
        printf("Warning: failed to open audio device: %s\n", SDL_GetError());
    }
    else
    {
...
Audio * createAudio(const char * filename, uint8_t loop, int volume)
{
    ...
    {
        printf("Warning: failed to open wave file: %s error: %s\n", filename, SDL_GetError());
        free(newAudio);
        return NULL;
    }
...
static inline void playAudio(const char * filename, Audio * audio, uint8_t loop, int volume)
{
    ...
    {
        printf("Warning: filename and Audio parameters NULL\n");
        return;
    }
